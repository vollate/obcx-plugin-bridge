...
auto MediaProcessor::get_qq_file_info(const std::string &file_url,
  ...
  try {
    ...
    OBCX_COMPONENT_ERROR("bridge", "获取QQ文件信息时出错: {}", e.what());
...
auto MediaProcessor::cleanup_media_file(const std::string &file_path) -> void {
  try {
    if (std::filesystem::exists(file_path)) {
      ...
      OBCX_COMPONENT_DEBUG("bridge", "清理临时媒体文件: {}", file_path);
    }
  } catch (const std::exception &e) {
    OBCX_COMPONENT_ERROR("bridge", "清理临时媒体文件失败: {}, 错误: {}",
                         file_path, e.what());
  }
}
  ...
  std::call_once(init_flag, []() -> void {
    if (bridge::config::BRIDGE_FILES_DIR.empty()) {
      throw std::runtime_error(
          ...
          "BridgeRuntimeConfig is registered before using MediaProcessor.");
    }
...
auto MediaProcessor::is_gif_content_type(const std::string &content_type)
  ...
  OBCX_COMPONENT_DEBUG("bridge", "Content-Type {} 判断为GIF: {}", content_type,
                       is_gif);
  return is_gif;
}
