...
auto PathManager::container_to_host_absolute(
  ...
  OBCX_COMPONENT_WARN(
      "bridge", "Container path '{}' is not within the mounted directory '{}'",
...
auto PathManager::ensure_directory(const std::string &relative_path) const
  ...
  try {
    ...
    OBCX_COMPONENT_DEBUG("bridge", "Ensured directory exists: {}", host_path);
    return true;
  } catch (const std::exception &e) {
    OBCX_COMPONENT_ERROR(
        "bridge", "Failed to create directory for relative path '{}': {}",
...
auto PathManager::normalize_path(const std::string &path) const -> std::string {
  ...
  try {
    ...
    OBCX_COMPONENT_WARN("bridge", "Failed to normalize path '{}': {}", path,
                        e.what());
    return path;
  }
}
