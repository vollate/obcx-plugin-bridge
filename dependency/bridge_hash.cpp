...
auto calculate_hash(const std::string &input) -> std::string {
  ...
  if (!context) {
    OBCX_COMPONENT_ERROR("bridge", "Failed to create EVP_MD_CTX");
    return "";
  }

  if (EVP_DigestInit_ex(context, EVP_sha256(), nullptr) != 1) {
    OBCX_COMPONENT_ERROR("bridge", "EVP_DigestInit_ex failed");
    EVP_MD_CTX_free(context);
    return "";
  }

  if (EVP_DigestUpdate(context, input.c_str(), input.length()) != 1) {
    OBCX_COMPONENT_ERROR("bridge", "EVP_DigestUpdate failed");
    EVP_MD_CTX_free(context);
    return "";
  }
  ...
  if (EVP_DigestFinal_ex(context, hash, &hash_len) != 1) {
    OBCX_COMPONENT_ERROR("bridge", "EVP_DigestFinal_ex failed");
    EVP_MD_CTX_free(context);
    return "";
  }
