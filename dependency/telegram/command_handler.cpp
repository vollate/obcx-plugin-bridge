...
auto TelegramCommandHandler::handle_recall_command(
  ...
  try {
    ...
    if (!reply_to_message.contains("message_id")) {
      OBCX_COMPONENT_WARN("tg_to_qq", "/recall 命令：无法获取被回复消息的ID");
      co_return;
    }
    ...
    OBCX_COMPONENT_INFO(
        "tg_to_qq", "/recall 命令：尝试撤回回复的Telegram消息 {} 对应的QQ消息",
        replied_message_id);
    ...
    OBCX_COMPONENT_INFO("tg_to_qq", "/recall 命令：找到对应的QQ消息ID: {}",
    ...
    try {
      ...
      if (recall_json.contains("status") && recall_json["status"] == "ok") {
        result_message = "✅ 撤回成功";
        OBCX_COMPONENT_INFO("tg_to_qq", "/recall 命令：成功在QQ撤回消息 {}",
                            target_qq_message_id.value());
        ...
        OBCX_COMPONENT_DEBUG("tg_to_qq", "已删除消息映射: telegram:{} -> qq:{}",
        ...
        OBCX_COMPONENT_WARN("tg_to_qq", "/recall 命令：QQ撤回消息失败: {}",
                            recall_response);
      }

    } catch (const std::exception &e) {
      OBCX_COMPONENT_ERROR("tg_to_qq", "/recall 命令：QQ撤回操作异常: {}",
                           e.what());
    ...
    try {
      ...
      OBCX_COMPONENT_ERROR("tg_to_qq", "/recall 命令：发送结果消息失败: {}",
                           send_e.what());
    }

  } catch (const std::exception &e) {
    OBCX_COMPONENT_ERROR("tg_to_qq", "处理 /recall 命令时出错: {}", e.what());
  }
}
...
auto TelegramCommandHandler::send_reply_message(
  ...
  try {
    ...
    OBCX_COMPONENT_ERROR("tg_to_qq", "发送回复消息失败: {}", e.what());
  }
}
...
auto TelegramCommandHandler::handle_checkalive_command(
  ...
  try {
    ...
    OBCX_COMPONENT_INFO("tg_to_qq", "/checkalive 命令处理完成");

  } catch (const std::exception &e) {
    OBCX_COMPONENT_ERROR("tg_to_qq", "处理 /checkalive 命令时出错: {}",
                         e.what());
...
auto TelegramCommandHandler::handle_poke_command(
  ...
  try {
    ...
    auto reply_to_message = event.data["reply_to_message"];
    if (!reply_to_message.contains("message_id")) {
      OBCX_COMPONENT_WARN("tg_to_qq", "/poke 命令：无法获取被回复消息的ID");
      co_return;
    }
    ...
    if (source_message.has_value()) {
      ...
      OBCX_COMPONENT_INFO("tg_to_qq", "/poke 命令：从消息记录找到QQ用户ID: {}",
                          target_qq_user_id);
    } else {
    ...
    OBCX_COMPONENT_INFO("tg_to_qq", "/poke 命令：准备戳QQ群 {} 的用户 {}",
                        std::string(qq_group_id), target_qq_user_id);

    auto *qq_bot_ptr = dynamic_cast<obcx::core::QQBot *>(&qq_bot);
    if (!qq_bot_ptr) {
      OBCX_COMPONENT_ERROR("tg_to_qq", "/poke 命令：无法获取QQBot实例");
      co_await send_reply_message(telegram_bot, telegram_group_id,
                                  event.message_id,
                                  "❌ 内部错误：无法获取QQ机器人实例");
      co_return;
    ...
    try {
      ...
      if (poke_json.contains("status") && poke_json["status"] == "ok") {
        OBCX_COMPONENT_INFO("tg_to_qq", "/poke 命令：成功在QQ群 {} 戳了用户 {}",
                            std::string(qq_group_id), target_qq_user_id);
        co_return;
      } else {
        ...
        OBCX_COMPONENT_WARN("tg_to_qq", "/poke 命令：戳一戳失败: {}",
                            poke_response);
      }

    } catch (const std::exception &e) {
      OBCX_COMPONENT_ERROR("tg_to_qq", "/poke 命令：戳一戳操作异常: {}",
                           e.what());
    ...
    try {
      ...
      OBCX_COMPONENT_ERROR("tg_to_qq", "/poke 命令：发送结果消息失败: {}",
                           send_e.what());
    }

  } catch (const std::exception &e) {
    OBCX_COMPONENT_ERROR("tg_to_qq", "处理 /poke 命令时出错: {}", e.what());
  }
}
