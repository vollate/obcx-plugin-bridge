...
auto TelegramEventHandler::handle_message_deleted(
  ...
  try {
    ...
    OBCX_COMPONENT_DEBUG("tg_to_qq", "Telegram消息删除事件处理尚未完全实现");
    co_return;

  } catch (const std::exception &e) {
    OBCX_COMPONENT_ERROR("tg_to_qq", "处理Telegram删除事件时出错: {}",
                         e.what());
  }
}
...
auto TelegramEventHandler::handle_message_edited(
  ...
  try {
    ...
    OBCX_COMPONENT_INFO("tg_to_qq", "处理Telegram群 {} 中消息 {} 的编辑事件",
    ...
    if (!target_message_id.has_value()) {
      OBCX_COMPONENT_DEBUG("tg_to_qq", "未找到Telegram消息 {} 对应的QQ消息映射",
                           event.message_id);
      co_return;
    }
    ...
    try {
      ...
      if (recall_json.contains("status") && recall_json["status"] == "ok") {
        OBCX_COMPONENT_INFO("tg_to_qq", "成功在QQ撤回消息: {}",
                            target_message_id.value());
        recall_success = true;
      } else {
        OBCX_COMPONENT_WARN("tg_to_qq", "QQ撤回消息失败: {}, 响应: {}",
                            target_message_id.value(), recall_response);
      }

    } catch (const std::exception &e) {
      OBCX_COMPONENT_WARN("tg_to_qq", "尝试在QQ撤回消息时出错: {}", e.what());
    }

    // 无论撤回是否成功，都尝试重发编辑后的消息
    OBCX_COMPONENT_INFO("tg_to_qq", "开始重发编辑后的消息到QQ (撤回状态: {})",
    ...
    try {
      ...
      OBCX_COMPONENT_INFO("tg_to_qq", "成功重发编辑后的消息");

    } catch (const std::exception &e) {
      OBCX_COMPONENT_ERROR("tg_to_qq", "重发编辑后的消息时出错: {}", e.what());

      if (recall_success) {
        OBCX_COMPONENT_WARN(
            "tg_to_qq", "撤回成功但重发失败，原QQ消息已被撤回但新消息发送失败");
      } else {
        ...
        OBCX_COMPONENT_WARN("tg_to_qq", "撤回和重发都失败，已删除消息映射");
      }
      co_return;
    }

  } catch (const std::exception &e) {
    OBCX_COMPONENT_ERROR("tg_to_qq", "处理Telegram编辑事件时出错: {}",
                         e.what());
  }
}
