...
auto TelegramHandler::forward_to_qq(obcx::core::IBot &telegram_bot,
  ...
  if (!media_group_id.empty() && !is_edited_resend_pre) {
    ...
    // Bot pointers are stable for the actor runtime's lifetime (host owns
    // them); the references we have here would dangle once this coroutine
    // returns so we capture by pointer. Capture a weak_ptr to ourselves so a
    // flush that runs after the handler is dropped becomes a no-op instead of a
    // use-after-free.
  ...
  if (it == GROUP_MAP.end()) {
    OBCX_COMPONENT_DEBUG("tg_to_qq", "Telegram群 {} 没有对应的QQ群配置",
                         telegram_group_id);
    co_return;
  }
  ...
  if (bridge_config->mode == BridgeMode::GROUP_TO_GROUP) {
    ...
    OBCX_COMPONENT_DEBUG("tg_to_qq", "群组模式：Telegram群 {} 转发到QQ群 {}",
                         telegram_group_id, qq_group_id);

    if (!bridge_config->enable_tg_to_qq) {
      OBCX_COMPONENT_DEBUG("tg_to_qq",
                           "Telegram群 {} 到QQ群 {} 的转发已禁用，跳过",
                           telegram_group_id, qq_group_id);
      co_return;
    }
  } else {
    ...
    if (!topic_config) {
      OBCX_COMPONENT_DEBUG(
          "tg_to_qq", "Telegram消息来自topic {}，没有对应的QQ群配置，跳过转发",
          message_thread_id);
      co_return;
    }
  ...
  if (event.raw_message.starts_with("/recall")) {
    OBCX_COMPONENT_INFO("tg_to_qq", "检测到 /recall 命令，处理撤回请求");
  ...
  if (event.raw_message.starts_with("/poke")) {
    if (GROUP_MAP.find(telegram_group_id) == GROUP_MAP.end()) {
      OBCX_COMPONENT_DEBUG("tg_to_qq",
                           "Telegram群 {} 不在配置中，忽略 /poke 命令",
                           telegram_group_id);
      co_return;
    }

    OBCX_COMPONENT_INFO("tg_to_qq", "检测到 /poke 命令，处理戳一戳请求");
  ...
  if (event.raw_message.starts_with("/")) {
    OBCX_COMPONENT_DEBUG("tg_to_qq", "忽略未处理的命令消息，不转发: {}",
  ...
  if (event.raw_message.starts_with("[QQ] ")) {
    OBCX_COMPONENT_DEBUG("tg_to_qq", "检测到可能是回环的QQ消息，跳过转发");
    co_return;
  }
  ...
  if (!is_edited_resend &&
      (state_repository_ ? state_repository_->get_target_message_id(
          ...
          .has_value()) {
    OBCX_COMPONENT_DEBUG("tg_to_qq", "Telegram消息 {} 已转发到QQ，跳过重复处理",
                         event.message_id);
    co_return;
  }

  OBCX_COMPONENT_INFO("tg_to_qq", "准备从Telegram群 {} 转发消息到QQ群 {}",
                      telegram_group_id, qq_group_id);
    ...
    if (event.data.contains("reply_to_message")) {
      ...
      if (reply_to_message.contains("message_id")) {
        ...
        if (!reply_to_message_id.has_value()) {
          ...
          OBCX_COMPONENT_DEBUG(
              "tg_to_qq", "移除reply_to_message字段，避免显示无效回复提示");
        }
    ...
    if (!message_to_send.empty()) {
      ...
      try {
        ...
        if (!qq_response.empty()) {
          OBCX_COMPONENT_DEBUG("tg_to_qq", "QQ API响应: {}", qq_response);
          ...
          if (response_json.contains("status") &&
            ...
            OBCX_COMPONENT_WARN(
                "tg_to_qq", "QQ响应格式错误，无法提取消息ID: {}", qq_response);
          }
        } else {
          failure_reason = "Empty response from QQ API";
          OBCX_COMPONENT_WARN("tg_to_qq", "QQ API返回空响应");
        }
      } catch (const std::exception &e) {
        failure_reason = fmt::format("Send failed: {}", e.what());
        OBCX_COMPONENT_WARN("tg_to_qq", "发送Telegram消息到QQ时出错: {}",
                            e.what());
      }
      ...
      if (!qq_message_id.has_value() && retry_manager_ &&
        ...
        OBCX_COMPONENT_ERROR("tg_to_qq", "消息发送失败且未启用重试: {}",
                             failure_reason);
      }
    }

  } catch (const std::exception &e) {
    OBCX_COMPONENT_ERROR("tg_to_qq", "处理Telegram到QQ转发时出错: {}",
                         e.what());
  }
...
auto TelegramHandler::forward_media_group_to_qq(
  ...
  if (it == GROUP_MAP.end()) {
    OBCX_COMPONENT_DEBUG("tg_to_qq",
                         "[media-group] Telegram群 {} 没有对应的QQ群配置",
                         telegram_group_id);
    co_return;
  }
  bridge_config = &it->second;
  ...
  if (existing_qq_message_id.has_value()) {
    ...
    OBCX_COMPONENT_DEBUG("tg_to_qq",
                         "media-group 已转发到QQ消息 {}，跳过重复发送",
                         existing_qq_message_id.value());
    co_return;
  }

  OBCX_COMPONENT_INFO(
      "tg_to_qq", "准备转发 Telegram media-group: {} 张消息, 群 {} -> QQ群 {}",
  ...
  try {
    ...
    if (!message_to_send.empty()) {
      ...
      try {
        ...
        failure_reason = fmt::format("Send failed: {}", e.what());
        OBCX_COMPONENT_WARN("tg_to_qq", "发送 media-group 到QQ时出错: {}",
                            e.what());
      }
      ...
      if (!qq_message_id.has_value() && retry_manager_ &&
        ...
        OBCX_COMPONENT_ERROR("tg_to_qq", "media-group 发送失败且未启用重试: {}",
                             failure_reason);
      }
    }
  } catch (const std::exception &e) {
    OBCX_COMPONENT_ERROR("tg_to_qq", "处理 Telegram media-group 转发时出错: {}",
                         e.what());
  }
