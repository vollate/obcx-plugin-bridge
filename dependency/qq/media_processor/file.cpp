...
auto QQMediaProcessor::process_file_segment(
  ...
  OBCX_COMPONENT_DEBUG("qq_to_tg", "转发QQ文件: file={}, url={}", file_name,
                       url);
  ...
  if (!url.empty()) {
    ...
    OBCX_COMPONENT_DEBUG("qq_to_tg", "使用QQ文件URL进行转发: {}", url);
  } else if (!file_id.empty()) {
    OBCX_COMPONENT_DEBUG("qq_to_tg", "URL为空，尝试通过file_id获取文件: {}",
                         file_id);
    try {
      std::string response;
      ...
      if (event.group_id.has_value()) {
        ...
        OBCX_COMPONENT_DEBUG("qq_to_tg", "get_group_file_url API响应: {}",
                             response);
      } else {
        std::string user_id = event.user_id;
        response = co_await qq_bot_ptr->get_private_file_url(user_id, file_id);
        OBCX_COMPONENT_DEBUG("qq_to_tg", "get_private_file_url API响应: {}",
                             response);
      }
      ...
      if (response_json.contains("status") && response_json["status"] == "ok" &&
        ...
        OBCX_COMPONENT_DEBUG("qq_to_tg", "成功通过API获取文件下载URL: {}",
                             download_url);
      } else {
        throw std::runtime_error("API响应中没有找到下载URL");
      }
    } catch (const std::exception &e) {
      OBCX_COMPONENT_WARN("qq_to_tg", "通过API获取文件URL失败: {}", e.what());
    ...
    OBCX_COMPONENT_WARN("qq_to_tg", "QQ文件缺少URL和file_id信息: {}",
                        file_name);
  }
